...
@Component({
  selector: 'app-grand-user',
  standalone: false,
  templateUrl: './grand-user.component.html',
  styleUrl: './grand-user.component.css',
})
export class GrandUserComponent {
  activeTab = 'details';
  today = new Date();
  tableData: TableRow[] = [
    {
    ...
    },
    {
      ...
      emailId: 'neha.verma@company.com',
    ...
    },
    {
      ...
      role: 'IT Support',
      emailId: 'sourabh.patil@company.com',
 ...
 loadGrandUsers(): void {
  ...
  this.commonService
    .fetchAllGrandUserByLoginId(this.loginId)
    .subscribe({
      ...
      error: (err) => {
        console.error('Grand User API Error:', err);
      }
...
deleteSelectedRows() {
  ...
  const ids = this.selectedRows.map(row => row.userId);

  this.commonService.deleteMultipleGrandUser(ids).subscribe({
  ...
  exportExcel() {
    ...
    const exportData = this.tableData.map((row) => ({
      ...
      Authorization_Date: row.authDate,
      Status: row.status,
    }));
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'UserAuthorizationData');

    // Download file
    XLSX.writeFile(workbook, 'UserAuthorizationData.xlsx');
  }
  ...
  <body>
    <div class="WordSection1">
      <table class="header-table">
        ...
        <tr>
          <td class="title">UserAuthorization Report</td>
        </tr>
      </table>
  ...
  `;
    ...
    saveAs(blob, 'UserAuthorization.doc');
  }
  ...
  exportPDF() {
    ...
    // Title center
    doc.setFontSize(18);
    doc.text('UserAuthorization Records', pageWidth / 2, 12, {
      align: 'center',
    });
    ...
    doc.save('UserAuthorizationData.pdf');
  }
  ...
  ngOnInit() {
    this.forms[0].newRecord.authDate = this.getTodayDate();
    this.loadGrandUsers();
  }
  ...
  addForm() {
    ...
    this.forms.push({
      newRecord: {
        ...this.newRecord,
        authDate: this.getTodayDate(), // auto-fill current date
      },
    });
...
// Step 1 → Mark form controls touched
if (form) {
...
if (this.isEditMode && this.editIndex !== -1) {
  ...
  const updatedRecord = this.forms[0].newRecord;

  const userId = this.tableData[this.editIndex].userId;
  ...
  this.commonService
    .updateGrandUser(prefix, year, code, this.loginId, updatedRecord)
    .subscribe({
...
const payload = this.forms.map((formItem) => {

  if (!formItem.newRecord.authDate) {
    formItem.newRecord.authDate = this.getTodayDate();
  }
  ...
  emailId: formItem.newRecord.emailId,
  contactNo: formItem.newRecord.contactNo,
  authDate: formItem.newRecord.authDate,
  status: formItem.newRecord.status || 'Active',
...
this.editIndex = -1;
  ...
  parseCSV(csv: string) {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const newRecord: TableRow = {
        ...
        role: obj['role'] || '',
        emailId: obj['emailId'] || obj['Email ID'] || '',
        contactNo: Number(obj['contactNo'] || obj['Contact No'] || 0),
        authDate: obj['authDate'] || obj['Authorization Date'] || '',
    ...
    reader.onload = () => {
      ...
      json.forEach((obj: any, i: number) => {
        const newRecord: TableRow = {
          ...
          contactNo: Number(obj['Contact No'] || 0),
          authDate: obj['Authorization Date'] || '',
          status: (obj['Status'] as 'Active' | 'Inactive') || 'Active',
        };
    ...
    reader.onload = () => {
      ...
      lines.forEach((line, idx) => {
        ...
        this.tableData.push({
          ...
          role: cols[3] || '',
          emailId: cols[4] || '',
          contactNo: Number(cols[5] || 0),
          authDate: cols[6] || new Date().toISOString().split('T')[0],
          status: ['Active', 'Inactive'].includes(cols[7])
            ? (cols[7] as 'Active' | 'Inactive')
            : 'Active',
        });
      });
  ...
  async readDOCX(file: File) {
    ...
    reader.onload = async () => {
      ...
      rows.forEach((row, rowIndex) => {
        ...
        const newRecord: TableRow = {
          userId: String(Number(cells[0]) || 0),
          userCode: cells[1]?.trim() || '',
          userName: cells[2]?.trim() || '',
          role: cells[3]?.trim() || '',
          emailId: cells[4]?.trim() || '',
          contactNo: Number(cells[5]?.trim() || 0),
          authDate: cells[6]?.trim() || new Date().toISOString().split('T')[0],
          status: ['Active', 'Inactive'].includes(cells[7])
            ? (cells[7] as 'Active' | 'Inactive')
            : 'Active',
        };
  ...
  getFile() {
    ...
    const filteredData = this.tableData.filter((row) => {
      const rowDate = this.parseDDMMYYYY(row.authDate);
  ...
  exportCSVfile(data: TableRow[]) {
    ...
    saveAs(blob, 'Filtered_UserAuthorization_Report.csv');
  }
  ...
  exportExcelfile(data: TableRow[]) {
    ...
    saveAs(blob, 'Filtered_UserAuthorization_Report.xlsx');
  }
  ...
  exportPDFfile(data: TableRow[]) {
    ...
    /* ==========================
     HEADER TITLE
  ========================== */
    const title = 'User Authorization Report';
    doc.setFontSize(22);
  ...
  ========================== */
    ...
    /* ==========================
     SAVE FILE
  ========================== */
    doc.save('User_Authorization_Report.pdf');
  }
}
